...
def heart_tracks
  ...
  execute(<<-SQL)
    ...
    FROM
      tracks
    JOIN
     albums AS album_list ON tracks.album = album_list.asin 
    WHERE
      tracks.song LIKE '%Heart%'
    GROUP BY 
      album_list.title
    Order BY COUNT(tracks.song) DESC, album_list.title ASC
--JOIN tracks AS h_list ON tracks.song LIKE 'Heart*'
  SQL
end
...
def expensive_tastes
  ...
  execute(<<-SQL)
    SELECT
      styles.style,
      SUM(album_counts.price) / SUM(album_counts.count)
    FROM
      styles
    JOIN (
      SELECT
    ...
    ) AS album_counts ON styles.album = album_counts.asin
    GROUP BY
      styles.style
    ORDER BY 
      SUM(album_counts.price) / SUM(album_counts.count) DESC, 
      styles.style ASC
    LIMIT 5
  SQL
end
