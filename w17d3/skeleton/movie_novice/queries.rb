...
def top_titles
  ...
  Movie.select('movies.title, id').where('score >= 9')
end
...
def below_average_years
  ...
  #"posts.*, COUNT(*) AS comments_count"
  Movie.select("movies.yr, COUNT(yr) AS bad_movies").where('score < 5').group(:yr).order(yr: :desc)
end
...
def alphabetized_actors
  ...
  Actor.select(:name, :id).order(:name).limit(10)
end
...
def uma_movies
  ...
  Movie.joins(:actors).where("actors.name = 'Uma Thurman'").select('movies.yr, movies.id, movies.title').order(yr: :desc)
end
